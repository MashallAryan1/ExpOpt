...
class GPRegressor(object):
    ...
    def fit(self, xt, yt):
        ...
        #define the kernel
        k = GPflow.kernels.Matern52(input_dim=xt.shape[1], ARD=self.ARD)
        k.lengthscales.transform = GPflow.transforms.Logistic(1e-5,50)
        k.variance.transform = GPflow.transforms.Logistic(0.5, 100)
        #define the mean
        self.meanf = GPflow.mean_functions.Zero
        #GP model
        self.model = GPflow.gpr.GPR(x, y, k, self.meanf())
        #likelihood
        self.model.likelihood.variance.transform = GPflow.transforms.Logistic(1e-2,0.1)
